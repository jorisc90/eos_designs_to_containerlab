        ...
        for switch in inventory:
            ...
            if "ethernet_interfaces" in hostvars[switch]:
                ...
                for eth in hostvars[switch]["ethernet_interfaces"]:
                    ...
                    # AVD 6.3 moved peer information under the interface metadata key.
                    # Keep the top-level fallback for structured configs generated by
                    # older AVD releases.
                    metadata = eth.get("metadata", {})
                    peer = eth.get("peer", metadata.get("peer"))
                    peer_interface = eth.get("peer_interface", metadata.get("peer_interface"))
                    peer_type = eth.get("peer_type", metadata.get("peer_type", "none"))
                    ...
                    if peer is not None and peer_interface is not None:

                        # Ignore connections to switches which are not deployed
                        if peer in hostvars:
                            if "is_deployed" in hostvars[peer] and hostvars[peer]["is_deployed"] == False:
                                continue
                        ...
                        if sim_env == "clab" and not containerlab_custom_interface_mapping:
                            ...
                            peer_int = str((peer_interface.split("."))[0]).replace("Ethernet","eth").replace("/","_")
                        else:
                            loc_int = str((eth["name"].split("."))[0])
                            peer_int = str((peer_interface.split("."))[0])

                        connection = {"loc_switch":switch, "loc_int":loc_int, "peer_name":peer, "peer_int":peer_int}

                        # Check if connection is not in the connections list already
                        if not self.dedup_connection(avd_connections, connection):
                            if peer in inventory:
                                ...
                                if sim_include_avd_external_nodes:
                                    ...
                                    if sim_external_node_one_container and not client_all_added:
                                        ...
                                        if connection["peer_name"] in hostvars:
                                            ext_nodes.append({peer:node_type})
